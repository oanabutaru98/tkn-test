...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var tripButton: Button
    lateinit var beaconReferenceApplication: BeaconReferenceApplication
    var alertDialog: AlertDialog? = null
    lateinit var regionViewModel : RegionViewModel
    var tripOngoing : Boolean = false

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)
        beaconReferenceApplication = application as BeaconReferenceApplication
        this.tripOngoing = false

        // Set up a Live Data observer for beacon data
        regionViewModel = BeaconManager.getInstanceForApplication(this).getRegionViewModel(beaconReferenceApplication.region)
//        // observer will be called each time the monitored regionState changes (inside vs. outside region)
//        regionViewModel.regionState.observe(this, monitoringObserver)
//        // observer will be called each time a new list of beacons is ranged (typically ~1 second in the foreground)
//        regionViewModel.rangedBeacons.observe(this, rangingObserver)
        tripButton = findViewById<Button>(R.id.tripButton)
        ...
        beaconCountTextView.text = "Trip has not started yet"
    ...
    fun tripButtonTapped(view: View) {
        val beaconManager = BeaconManager.getInstanceForApplication(this)
        if (tripOngoing == false) {
            tripOngoing = true
//        if (beaconManager.rangedRegions.size == 0) {
            ...
            beaconCountTextView.text = "AWAITING BEACONS"
            //beaconListView.adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, arrayOf("waiting..."))
        }
        else {
            tripOngoing = false
